  ...
  return (
    <section className="flex w-full items-start bg-vanilla overflow-hidden">
      ...
      <div className="flex flex-col items-center justify-between pt-[100px] pb-[70px] px-[100px] self-stretch relative w-1/2">
        <h2
          className={`self-stretch font-titres ${currentState.titleColor} text-[length:var(--titres-font-size)] leading-[var(--titres-line-height)] relative mt-[-1.00px] font-[number:var(--titres-font-weight)] text-center tracking-[var(--titres-letter-spacing)] [font-style:var(--titres-font-style)] transition-opacity duration-700 ${
            fade ? "opacity-100" : "opacity-0"
          }`}
        >
          {currentState.title}
        ...
        <div className="flex items-center justify-center gap-2.5 px-0 py-0.5 self-stretch w-full flex-[0_0_auto] rounded-[10px]">
          <Button
            className={`flex items-center justify-center gap-2.5 pt-[5px] pb-[7px] px-[25px] relative flex-1 ${currentState.buttonColor} rounded-[10px]  shadow-[0px_1px_2px_#00000040] h-auto hover:underline hover:-translate-y-1 transition-all duration-300`}
          >

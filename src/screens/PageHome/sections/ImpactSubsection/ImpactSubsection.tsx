...
export const ImpactSubsection = (): JSX.Element => {
                          ...
                          <div className="flex flex-col gap-10">
                            {Array.isArray(stats.left) ? (
                              ...
                              <div className="flex flex-col items-center text-pumpkin">
                                <div className="text-[50px] font-bold font-[beautique-display]">
                                  <Counter
                                    value={parseInt(stats.left.number, 10)}
                                    duration={1000}
                                  />
                                </div>
                                <div className="text-[32px] font-[beautique-display]">
  ...
  // ================== MOBILE VERSION ==================
  return (
    <section className="block xl:hidden w-full bg-colbat py-16 relative">
      {" "}
      {/* relative ajouté ici */}
      <header className="flex flex-col items-center text-center gap-6">
        <h2 className="text-antiflash font-titres text-3xl">
          NOTRE IMPACT <br />
          <span className="font-[beautique-display] font-normal">en</span>{" "}
          CHIFFRES
        </h2>
      ...
      <div className="mt-10 w-full">
        <Swiper
          ...
          onSlideChange={(swiper) => {
            // 🔥 Quand on change de slide, on force la remontée du compteur
            const activeIndex = swiper.activeIndex;
            const activeSlide = swiper.slides[activeIndex];
            if (activeSlide) {
              const counters = activeSlide.querySelectorAll("[data-counter]");
              // Import createRoot from react-dom/client at the top of the file
              counters.forEach((el: any) => {
                const value = parseInt(el.getAttribute("data-value"), 10);
                if (!isNaN(value)) {
                  // On recrée le Counter dynamiquement
                  el.innerHTML = "";
                  const mountPoint = document.createElement("span");
                  el.appendChild(mountPoint);
                  // React réinjecte Counter dans l'élément
                  // Use createRoot directly
                  createRoot(mountPoint).render(
                    <Counter value={value} duration={1000} />
                  );
                }
              });
            }
                  ...
                  {items.map((stat, i) => (
                    <div key={i} className="flex w-full">
                      <div className="flex-1 text-pumpkin text-center">
                        <div
                          className="text-3xl font-bold font-[beautique-display]"
                          data-counter
                        ...
                        <div className="text-lg font-[beautique-display]">
                          {stat.title}
                        </div>
                      </div>
                      <div className="flex-1 text-pumpkin text-center">
      ...
      <div className="mt-20 flex justify-center relative z-10">
        {" "}
        {/* relative ici pour que le bouton se place correctement */}
        <DonateButton />
      </div>
